...
from .bulk import create, COMING_IN
...
def load_genres():
    ...
    ds_movie_genre.add_values(MOVIE_GENRES)
    ds_music_genre.add_values(set(MUSIC_GENRES) - similar)
    for value in similar:
        ds_music_genre.add_value(value)
...
def load_genres_batch():
    ...
    create((
        (['Dataset'], {'name': 'Movie genre'}, [('VALUE_OF', COMING_IN, (
            (['DatasetValue'], {'name': genre}) for genre in MOVIE_GENRES)
        )]),
        (['Dataset'], {'name': 'Music genre'}, [('VALUE_OF', COMING_IN, (
            (['DatasetValue'], {'name': genre}) for genre in set(MUSIC_GENRES) - similar)
        )])
    ))
    ds_music_genre = Dataset.get(name='Music genre')
    for value in similar:
        ds_music_genre.add_value(value)
